...
def align_private_tensor(
    ...
    return sliced.to(device=device, dtype=torch.float32)
...
def scale_params_weekly(params_epi_weekly, param_ranges):
    if not param_ranges or params_epi_weekly is None:
        return params_epi_weekly
    out = params_epi_weekly.clone()
    for i, key in enumerate(PARAM_ORDER):
        if key not in param_ranges or len(param_ranges[key]) != 2:
            continue
        lo, hi = float(param_ranges[key][0]), float(param_ranges[key][1])
        out[:, i] = lo + (hi - lo) * out[:, i]
    return out


def enforce_epi_constraints(params_weekly: torch.Tensor) -> torch.Tensor:
    """
    Ensure biologically valid parameter ranges to prevent simulator blow-ups.
    Clamps to [0,1] and enforces gamma + mor <= 1 while preserving their ratio.
    """
...
def forward_simulator(
    ...
    if param_ranges is not None:
        params_epi_weekly = scale_params_weekly(params_epi_weekly, param_ranges)
    params_epi_weekly = enforce_epi_constraints(params_epi_weekly)
    preds = []
...
def main():
    ...
    ap.add_argument(
        "--long_train",
        action="store_true",
        help="Use long-training regimen (num_epochs_long from config if present, else 5x num_epochs_diff).",
    )
    ...
    ap.add_argument("--self_check", action="store_true", help="Print key tensor shapes for quick debugging.")
    ...
    if use_private and private_full is not None:
        private_t = private_full.shape[1] if private_full.dim() == 2 else private_full.shape[1]
    ...
    trainable_params = list(param_model.parameters())
    if error_adapter is not None:
        trainable_params += list(error_adapter.parameters())
    ...
    if args.long_train:
        num_epochs = int(cfg.get("num_epochs_long", cfg["num_epochs_diff"] * 5))
    ...
    for _ in pbar:
        ...
        for (X_train_b, y_train_b) in train_loader:
            ...
            training_num_steps = y_train.shape[0]
            ...
            # Hard assertions to fail fast on bad shapes or NaNs
            ...
            base_preds_train = forward_simulator(
                ...
                param_ranges=nyc.get("param_ranges"),
            )

            if error_adapter is not None:
                preds_train = base_preds_train + error_adapter(base_preds_train)
            else:
                preds_train = base_preds_train

            assert preds_train.shape == y_train_s.shape
            assert torch.isfinite(preds_train).all()

            loss = torch.sqrt(mse(preds_train, y_train_s))
            opt.zero_grad()
            loss.backward()
            if args.clip_norm is not None and args.clip_norm > 0:
                torch.nn.utils.clip_grad_norm_(trainable_params, args.clip_norm)
            opt.step()

            loss_val = float(loss.detach().cpu().item())
            epoch_best = min(epoch_best, loss_val)

        if epoch_best < best_loss:
        ...
        pbar.set_postfix_str(f"RMSE={best_loss:.4f}")
